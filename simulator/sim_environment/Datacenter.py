...
class DatacenterGeneration:
    def __init__(self, nums: dict, metrics: dict, container_conf: dict, num_steps: int,
                 ...
                 seed: int, contexts: str, images: list, versions: list, config_path: str):
        ...
        self.model_image = dict(zip(versions, images))
        self.pulled = False
        self.cost = 0
        self.locations = []
        self.port = 5000
        ...
        assert len(self.container_conf['0']) == len(metrics), \
    ...
    def warmup(self):
        ...
        for cluster in self.clusters:
            ...
            for node in range(nodes_in_cluster):
                # now Deploy each image in each node once and destroy it
                node_name = cluster['nodes'][node]['node_name']
                for id, image in enumerate(self.images):
                    service_name = generate_random_service_name(service_id=id, node_id=node)
                    service_body = construct_service(name=service_name, port=5000, targetPort=5000)
                    pod_body = construct_pod(name=service_name, image=image, limit_cpu=f"{cpu}m",
    ...
    def randomDeployment(self) -> bool:
        ...
        cpu_utilization = self.hosts_resources_util[:, 1]
        # Get the indices that would sort the CPU utilization values in descending order
        sorted_indices = np.argsort(-cpu_utilization)
        # Sort the nodes by these indices
        sorted_nodes = self.hosts_resources_util[sorted_indices]
        host_list = list(sorted_nodes[:,0])
        try:
            ...
            sys.exit(1)  # Terminate the program with a status code indicating an error
        ...
        return self.ip_model_image, cost
    ...
    def initialization_new(self, prev_decision):
        ...
        for service_id, node in enumerate(self.containers_hosts_obj):
            if prev_decision[service_id] != self.containers_hosts_tuple[service_id]:
                ...
                service = KubeService(id=service_id, pod=pod, svc=svc, model=version)
                ...
                self.containers_ips.append(service_ip)
                self.containers_versions.append(version)
                self.containers_images.append(image)
        ...
        self.ip_model_image = dict(enumerate(zip(self.containers_ips, self.containers_versions, self.containers_images)))
    ...
    def migrate(self, prev_decision, prev_containers_hosts_obj): 
        ...
        df_host_capacity = pd.DataFrame(self.hosts_resources_cap)
        df_hosts_remaining = pd.DataFrame(self.hosts_resources_remain)
        df_host_alloc = pd.DataFrame(self.hosts_resources_alloc)
        df_hosts_requested = pd.DataFrame(self.hosts_resources_req)
        self.host_alloc_list.append(df_host_alloc)
        self.host_capacity_list.append(df_host_capacity)
    ...
    def gkems_migrate(self, prev_decision, prev_containers_hosts_obj): 
        ...
        self.containers_hosts_obj = self.kube_nodes[self.containers_hosts]
        prev_node_names = deepcopy(self.gke_node_name)
        ...
        for idx, (service, node) in enumerate(zip(self.services, self.containers_hosts_obj)):
            if (prev_decision[idx] != self.containers_hosts_tuple[idx]) or (service.model != self.ip_model_image[idx][1]):
                ...
                modified_tuple = (service_ip, temp_tuple[1], temp_tuple[2])
                self.ip_model_image[idx] = modified_tuple
                self.gke_node_name[idx] = pod.spec.node_name
                self.containers_locations[idx] = node.location
        ...
        df_host_alloc = pd.DataFrame(host_alloc)
        df_hosts_requested = pd.DataFrame(hosts_requested)
        self.host_alloc_list.append(df_host_alloc)
        self.host_capacity_list.append(df_host_capacity)
        self.hosts_remaining_list.append(df_hosts_remaining)
        self.hosts_requested_list.append(df_hosts_requested)
                            ...
                            for pod in pods:
                                for container in pod.spec.containers:
                                    ...
                                    if container.resources.requests:
                                        ...
                                        #print(f"Namespace: {pod.metadata.namespace}, CPU: {cpu_request}")
                                        ...
                                        if memory_request:
                                            ...
                                            if match:
                                                ...
                                                #if pod.metadata.namespace == 'prokube':
                                                #    total_mem_requests_prokube += memory_value
                        else:
